...
class BotStates(StatesGroup):
    ...
    menu_act = State()
    act_number = State()
    neiyrosearhe = State() # cосстояние после нажание в кнопки неропомощник 
...
def handle_text(message): # функция для обработки текстовых сообщений
    ...
    if text == '📝Рег. несоответствия':
     ...
     #Прописываем следующий шаг, который ожидает ввод номера акта
        ...
        bot.send_message(chat_id, 'Я помошник РПРЗ задавайте свой вопрос:', reply_markup=back_markup)
       # 2. Переводим пользователя в состояние ожидания вопроса для нейропомощника
        bot.register_next_step_handler(message, handle_neiro)  # Переводим в состояние ожидания вопроса
        print(f"Пользователь {chat_id} переведен в состояние neiyrosearhe")  # Выводим в консоль для отладки
...
# === Обработчик для вопросов в состоянии "Нейропомощник РПРЗ" ===
@bot.message_handler(state=BotStates.neiyrosearhe, content_types=['text'])
def handle_neiro(message):
    chat_id = message.chat.id # Получаем ID чата
    user_question  = message.text
    if user_question == 'Назад':
        # Возвращаем пользователя в главное меню
        bot.set_state(message.from_user.id, BotStates.main_menu, chat_id) # Сбрасываем состояние
        bot.send_message(chat_id, "Вы вернулись в главное меню.") # Отправляем главное меню
        print(f"Пользователь {chat_id} вернулся в главное меню из neiyrosearhe.")
        return # Выходим из функции, чтобы не обрабатывать как вопрос
    bot.send_message(chat_id, f"Спасибо за вопрос: '{user_question}'. Я пока не умею отвечать, но скоро научусь!")
...
def extract_text_from_pdf(pdf_path):
    ...
    Returns:
        str: Весь извлеченный текст или пустая строка в случае ошибки.
    """
    text = "" 
    try:
        # Открываем PDF-файл в бинарном режиме для чтения ('rb')
        with open(pdf_path, 'rb') as file:
            # Создаем объект PdfReader.
            reader = PyPDF2.PdfReader(file) 
            
            # Проходим по каждой странице в документе
            for page_num in range(len(reader.pages)):
                page = reader.pages[page_num] # Получаем объект страницы
                page_text = page.extract_text() # Извлекаем текст со страницы
                if page_text: # Проверяем, что текст на странице не пустой
                    text += page_text + "\n" # Добавляем текст страницы и разделитель
    except PyPDF2.errors.PdfReadError:
        logger.error(f"Ошибка чтения PDF-файла (возможно, поврежден или защищен): {pdf_path}")
        return "" # Возвращаем пустую строку в случае ошибки чтения PDF
    except FileNotFoundError:
        logger.error(f"PDF-файл не найден: {pdf_path}")
        return "" # Возвращаем пустую строку, если файл не найден
    except Exception as e: # Общий обработчик для любых других неожиданных ошибок
        logger.error(f"Неизвестная ошибка при извлечении текста из {pdf_path}: {e}")
        return ""
    return text
