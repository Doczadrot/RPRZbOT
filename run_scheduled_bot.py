...
def main():
    ...
    while True:
        ...
        if return_code is not None and return_code != 0:
            logger.error(f"❌ Бот завершился с ошибкой: {return_code}")
            # Ждём немного перед повторной попыткой
            time.sleep(30)

        # После остановки бота ждём следующего рабочего дня
        logger.info("💤 Переход в режим ожидания следующего рабочего дня")
